...
def gross_per_studio(collection)
  ...
  while i < collection.length do
    ...
    h[studio] = h.fetch(studio,0) + collection[i][:worldwide_gross]
    i += 1
  end
  return h
end
...
def movies_with_directors_set(source)
  ...
  while director_count < source.length do
    movie_count = 0
    director = source[director_count][:name]
    inside_array = []
    while movie_count < source[director_count][:movies].length do
      h = source[director_count][:movies][movie_count]
      h[:director_name] = director
      inside_array << h
      movie_count += 1
    end
    a << inside_array
